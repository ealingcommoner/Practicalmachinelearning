...
```{r lda}
model2lda<- train(classe ~.,method="lda",data=training)
pred2lda<- predict(model2lda, validation)
confusionMatrix(pred2lda, validation$classe)$overall["Accuracy"] 
```
Now let's build a basic decision tree. Decision trees are fast and can handle data with non-gaussian distributions.  
```{r boosted tree}
...
```{r random forest}
model2rf<- randomForest(classe ~.,data=training, ntree=100) 
pred2rf<- predict(model2rf, validation)
confusionMatrix(pred2rf, validation$classe)$overall["Accuracy"] 
```
This model performs well with 99.6% accuracy. 
...
```
# Out of sample error
So our model is 99.6% accurate. We would therefore predict an out of sample error of 0.4%.
